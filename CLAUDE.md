...
`packages/shared` defines the contract between server and web (`AgentStreamEvent`, `Message`/`MessagePart`, `Session`/`SessionView`) — both sides import from it, never redefine these shapes locally. It also owns the HTTP API's Zod request schemas and error envelope (`apiSchemas.ts`, `apiError.ts`; ADR-0039). The rule there is **schema what crosses the wire inbound, type what goes out**: request bodies and query params get Zod schemas, domain shapes the server produces stay plain types. Validate with `validate()` from `apps/server/src/routes/factory.ts`, never `zValidator` directly — it's what attaches the shared error envelope. Every failure returns `{ error: { message, status, fieldErrors? } }` via `app.onError`; route handlers keep throwing `HTTPException` and that decides the rendering.
