...
`packages/shared` defines the contract between server and web (`AgentStreamEvent`, `Message`/`MessagePart`, `Session`/`SessionView`) — both sides import from it, never redefine these shapes locally. It also owns the HTTP API's Zod request schemas, response envelopes and error envelope (`apiSchemas.ts`, `apiResponses.ts`, `apiError.ts`; ADR-0039/0040). The rule there is **schema what crosses the wire inbound, type what goes out**: request bodies and query params get Zod schemas, domain shapes the server produces stay plain types. Validate with `validate()` from `apps/server/src/routes/factory.ts`, never `zValidator` directly — it's what attaches the shared error envelope. Response envelopes (the `{ repos: [...] }` wrapper, not the domain shape inside) live in `apiResponses.ts` and must be **named on both sides** — the route annotates `const body: RepoResponse = ...`, the client writes `request<RepoResponse>(...)` — never restated inline. The compiler can't enforce that convention by itself (narrowing a client generic breaks nothing unless some consumer reads the dropped field, which is how three envelopes silently drifted), so `apps/web/src/api/client.contract.test-d.ts` pins it; run it with `pnpm test:types`. Every failure returns `{ error: { message, status, fieldErrors? } }` via `app.onError`; route handlers keep throwing `HTTPException` and that decides the rendering.

`apps/server/src/agents/` holds one file per `Agent` backend — today that's a single file, `pi.ts`, built on `pi-agent-core`'s bare `Agent` class (see ADR-0020 for the go decision to replace the prior `claude.ts`/Claude-Agent-SDK backend with it, and ADR-0011 for why dilna standardizes on one backend file rather than a `handle.kind` union of several — `sessions/manager.ts` dispatches to it directly). Adding a second backend means a new adapter file plus a new dispatch branch in `manager.ts` — not a new abstraction layer (see ADR-0002/0007 for why). The adapter normalizes its backend's native event/transcript shape into the shared `AgentStreamEvent`/`Message` types itself; read `pi.ts`'s own comments before touching it — they document backend-specific quirks (worktree confinement via a `beforeToolCall` hook, the pi-side persistence bridge) that aren't repeated here. Policy that merely *needs* a model to run has been pulled out to where it belongs: the Worktree sandbox/toolchain grant to `agents/worktreeSandbox.ts` (issue #177) and the context/compaction/archive policy to `sessions/context.ts` (issue #175, addenda on ADR-0023/0024). Both left the adapter as the imported side, not the importing one — `sessions/context.ts` calls `pi.ts`'s narrow `resolveSummarizationModel`/`summarizeMessages`, and nothing in the compaction path reaches back into a live `PiHandle`.

`SessionManager` (`apps/server/src/sessions/manager.ts`) owns the full session lifecycle: worktree creation via `git worktree add`, spawning/resuming the agent process, idle-timeout kill, and persisting agent-native messages into dilna's own SQLite tables after each turn (which of a turn's rows are already durable is `sessions/turnLedger.ts`'s job, not a field on the agent record — see ADR-0035) (the DB is the source of truth for history; live agent processes are disposable). The SSE stream in `routes/stream.ts` and `ChatShell.tsx`'s `live` state are a rendering optimization on top of that, not a second source of truth.
