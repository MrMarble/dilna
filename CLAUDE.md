...
`apps/server/src/agents/` holds one file per `Agent` backend (currently `opencode.ts` and `claude.ts`); `sessions/manager.ts` is the only place that dispatches between them, via a `handle.kind` discriminant on the union of their handle types. Adding a backend means a new adapter file plus a new dispatch branch there — not a new abstraction layer (see ADR-0002/0007 for why). Each adapter normalizes its backend's native event/transcript shape into the shared `AgentStreamEvent`/`Message` types itself; read the adapter file's own comments before touching it — they document backend-specific quirks (e.g. `claude.ts`'s streaming-input readiness behavior, transcript-write lag) that aren't repeated here.
