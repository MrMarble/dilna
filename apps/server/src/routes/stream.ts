...
export function createStreamRoute(deps: { sessions: SessionManager }): Hono {
	...
	streamRoute.get("/", (c) => {
		return streamSSE(c, async (stream) => {
			...
			const rateLimitWindows = deps.sessions.getRateLimits();
			...
			await runSseLoop(stream, c.req.raw.signal, (push) => {
				return deps.sessions.subscribeAll((ev) => {
