 ...
 * Everything else on the host is readable (`-r /`) but not writable,
 * enforced via Landlock + seccomp rather than a mount namespace.
 *
 ...
 *
 * Read access is deliberately *not* narrowed with `--fs-deny` (a sibling-deny
 * scheme was tried and reverted — see ADR-0010's "Read confinement" section):
 * any `--fs-deny` rule at all makes the Claude Agent SDK's Bun-compiled CLI
 * exit immediately with no output, regardless of which path is denied. Write
 * confinement — the boundary the original dogfooding incident actually
 * needed — is unaffected by that bug and is what this function enforces.
 *
 ...
 * listening) only — not outbound egress, read access, or process visibility.
 */
